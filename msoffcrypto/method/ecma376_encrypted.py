...
#
# Initial C++ code from https://github.com/herumi/msoffice
#
...
class Header:
    ...
    def writeTo(self, obuf):
        ...
        obuf.write(pack("<HHHHHHHHIIIIIIIII", self.minorVersion, self.majorVersion, byteOrder, self.sectorShift, miniSectorShift, reserved, reserved, reserved,
                        self.numDirectorySectors, self.numFatSectors, self.firstDirectorySectorLocation, self.transactionSignatureNumber, miniStreamCutoffSize,
                        self.firstMiniFatSectorLocation, self.numMiniFatSectors, self.firstDifatSectorLocation, self.numDifatSectors))
...
class DirectoryEntry:
    def __init__(self, name="", _type=DirectoryEntryType.EMPTY, color=RedBlack.RED, leftId=SectorTypes.NOSTREAM,
                 rightId=SectorTypes.NOSTREAM, childId=SectorTypes.NOSTREAM, clsid="", bits=0, ct=0, mt=0,
                 loc=0, content=b""):
        self.Name = name
        self.Type = _type
        self.Color = color
    ...
    def writeHeaderTo(self, obuf):
        ...
        self.writeFiletime(obuf, self.CreationTime)
        self.writeFiletime(obuf, self.ModificationTime)

        obuf.write(pack("<IQ", self.StartingSectorLocation, len(self.Content)))

    def writeFiletime(self, obuf, ft):
...
class ECMA376Encrypted:
    def __init__(self, encryptedPackage=b"", encryptionInfo = b""):
        ...
        self._dirs = self._getDirectoryEntries()

        self.setPayload(encryptedPackage, encryptionInfo)
    ...
    def setPayload(self, encryptedPackage, encryptionInfo):
    ...
    def _getDirectoryEntries(self):
        ...
        directories = [ # Must follow DSPos ordering
            ...
            DirectoryEntry("DataSpaceMap",              DirectoryEntryType.STREAM,       RedBlack.BLACK, ct=ft, mt=ft, leftId=DSPos.iVersion, rightId=DSPos.iDataSpaceInfo, content=DefaultContent.DataSpaceMap),
            DirectoryEntry("DataSpaceInfo",             DirectoryEntryType.STORAGE,      RedBlack.BLACK, ct=ft, mt=ft, rightId=DSPos.iTransformInfo, childId=DSPos.iStongEncryptionDataSpace),
            DirectoryEntry("StrongEncryptionDataSpace", DirectoryEntryType.STREAM,       RedBlack.BLACK, ct=ft, mt=ft, content=DefaultContent.StrongEncryptionDataSpace),
            DirectoryEntry("TransformInfo",             DirectoryEntryType.STORAGE,      RedBlack.RED,   ct=ft, mt=ft, childId=DSPos.iStrongEncryptionTransform),
            ...
            DirectoryEntry("EncryptionInfo",            DirectoryEntryType.STREAM,       RedBlack.BLACK, ct=ft, mt=ft, leftId=DSPos.iDataSpaces, rightId=DSPos.iEncryptionPackage)
        ]

        return directories

    def _writeTo(self, obuf):
        layout = ECMA376EncryptedLayout(self._header.sectorSize)

        self._setSectorLocationsOfStreams(layout)
        self._detectSectorNum(layout)
        ...
        self._header.writeTo(obuf)

        self._writeDifat(obuf, layout)
        self._writeFatStart(obuf, layout)
        self._writeMiniFat(obuf, layout)

        self._writeDirectoryEntries(obuf, layout)
        self._writeContent(obuf, layout)

    def _writeDirectoryEntries(self, obuf, layout: ECMA376EncryptedLayout):
        obuf.seek(layout.offsetDirectoryEntries)

        for d in self._dirs:
            d.writeHeaderTo(obuf) # This must write 128 bytes, no more, no less.
    ...
    def _writeContent(self, obuf, layout: ECMA376EncryptedLayout):
        for d in self._dirs:
            size = len(d.Content)
    ...
    def _writeFatStart(self, obuf, layout: ECMA376EncryptedLayout):
        ...
        self._writeFat(obuf, v, layout.fatSectorNum * layout.sectorSize)

    def _writeMiniFat(self, obuf, layout: ECMA376EncryptedLayout):
        obuf.seek(layout.offsetMiniFat)
        self._writeFat(obuf, layout.miniFatSectors, layout.numMiniFatSectors * layout.sectorSize)

    def _writeFat(self, obuf, entries, blockSize):
        v = 0
        ...
        if obuf.tell() - startPos != blockSize:
            raise Exception("_writeFat did not completely fill the block space.")

    def _writeDifat(self, obuf, layout: ECMA376EncryptedLayout):
    ...
    def _detectSectorNum(self, layout: ECMA376EncryptedLayout):
        ...
        for i in range(10):
            a = self._getBlockNum(difatSectorNum + fatSectorNum + layout.contentSectorNum, numInFat)
            b = 0 if a <= Header.FIRSTNUMDIFAT else self._getBlockNum(a - Header.FIRSTNUMDIFAT, numInFat - 1)
    ...
    def _setSectorLocationsOfStreams(self, layout: ECMA376EncryptedLayout):
        ...
        for s in streamsOfInterest:
            n = self._getMiniFatSectorNumber(len(s.Content))
        ...
        miniFatNum = pos
        miniFatDataSectorNum = self._getBlockNum(miniFatNum, (self._header.sectorSize // 64))

        if self._getBlockNum(miniFatDataSectorNum, 128) > 1:
        ...
        layout.directoryEntrySectorNum     = self._getBlockNum(len(self._dirs), 4)
        layout.encryptionPackageSectorNum  = self._getBlockNum(len(self._dirs[DSPos.iEncryptionPackage].Content), layout.sectorSize)

    def _getMiniFatSectorNumber(self, size):
        return self._getBlockNum(size, 64)

    def _getBlockNum(self, x, block):
